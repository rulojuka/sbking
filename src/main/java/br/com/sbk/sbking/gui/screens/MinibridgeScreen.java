...
public class MinibridgeScreen extends GameScreen implements SBKingScreen {
    ...
    while (true) {
      ...
      if (sbkingClient.isSpectator()) {
        if (sbkingClient.getBoardHasChanged() || sbkingClient.getDealHasChanged()
          ...
          Painter painter;
          if (currentDeal == null) {
            painter = this.painterFactory.getSpectatorPainter(currentBoard, sbkingClient.getPlayCardActionListener());
          } else {
            painter = this.painterFactory.getSpectatorPainter(currentDeal, sbkingClient.getPlayCardActionListener());
          }
